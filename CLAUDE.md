...
- **Home intro shootout** (`src/components/HomeIntro.tsx`): the home page opens on a penalty shootout the visitor *plays*, they click a corner of the goal to shoot, a keeper dives, and a scoreboard tallies goals against saves over five shots. Winning shows a celebration screen (confetti plus a trophy); either way an "Enter site" button (and an always-present Skip) fades the overlay out to reveal the hero. Runs only once per browser session (`sessionStorage`), skipped entirely for reduced-motion visitors, and the hero always renders in the server HTML underneath so the intro never gates content or hurts SEO. Framer Motion drives the ball/keeper/confetti; per-shot timing is a short timer chain triggered by each click. (This was auto-play at one point; Kasra asked for it to be interactive with a win celebration, so don't revert it to auto-play.)
